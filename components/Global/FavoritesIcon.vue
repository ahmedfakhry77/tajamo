  ...
  <div class="relative">
    ...
    >
      ...
      <div class="p-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">{{ $t('favorites.header.title') }}</h3>
      </div>

      <!-- Favorites Items -->
      <div v-if="favoritesStore.hasFavorites" class="max-h-64 overflow-y-auto">
        <div
          v-for="productId in favoritesStore.favorites.slice(0, 3)"
          :key="productId"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm font-medium text-gray-900">
              {{ getProductPrice(productId) }} {{ getProductCurrency(productId) }}
            </p>
          </div>
        ...
        <div v-if="favoritesStore.favorites.length > 3" class="p-4 text-center">
          <p class="text-sm text-gray-500">
            {{ $t('common.view') }} {{ favoritesStore.favorites.length - 3 }} {{ $t('common.more') }}...
          </p>
        </div>
      </div>
      ...
      <!-- Footer -->
      <div v-if="favoritesStore.hasFavorites" class="p-4 border-t border-gray-200">
        <NuxtLink
          to="/favorites"
