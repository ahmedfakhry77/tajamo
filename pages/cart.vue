<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4">
      <!-- Breadcrumb -->
      <Breadcrumb :breadcrumbs="[{ name: $t('navigation.cart'), path: '/cart' }]" />
      
      <!-- Page Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $t('cart.header.title') }}</h1>
      ...
      <div v-else-if="!cartStore.hasItems" class="text-center py-12">
        ...
        <h2 class="text-xl font-semibold text-gray-900 mb-2">{{ $t('cart.empty.title') }}</h2>
      ...
      <!-- Cart Items -->
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-2">
        <!-- Cart Items List -->
        <div class="lg:col-span-2">
          <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200">
              <h2 class="text-lg font-semibold text-gray-900">{{ $t('cart.items.title', { count: cartStore.uniqueItemsCount }) }}</h2>
            </div>
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-lg font-semibold text-gray-900 mt-2">
                    {{ getProductPrice(item.productId) }} {{ getProductCurrency(item.productId) }}
                  </p>
                </div>
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-8">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('cart.summary.title') }}</h2>
            
            <div class="space-y-3 mb-6">
              <div class="flex justify-between text-sm text-gray-600">
                <span>{{ $t('cart.summary.subtotal', { count: cartStore.cartItemsCount }) }}</span>
            ...
            >
              <div v-if="cartStore.isLoading" class="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
              {{ cartStore.isLoading ? $t('cart.actions.processing') : $t('cart.actions.checkout') }}
            </button>
